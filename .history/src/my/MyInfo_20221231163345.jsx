...
const MyInfo = () => {
  ...
  const onCheckUnickanme = async (e) => {
    ...
    const result = await axios.get(`/api/user/data/${loginUser.unickname}`);
    result.data === 1 ?
  ...
  const fetchUserInfo = async () => {
    setLoading(true);
    const result = await axios.get(`/api/user/${uid}`);
  ...
  const onUpdate = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          await axios.post('/api/user/update', data, config);
          Swal.fire({
            text: "수정을 완료하였습니다!",
            icon: 'success',
            confirmButtonColor: '#3085d6',
  ...
  const onDelete = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await axios.patch('/api/user/', uid);
