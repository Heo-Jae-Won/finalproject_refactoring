...
const navigate=useNavigate();
  ...
  const callLikeList = useCallback(async () => {
    const result = await axios.get(`/api/pboard/like/${pcode}/${loginUser.unickname}`)
  ...
  const onLikeClick = async () => {
    ...
    likeCliked === false ?
      await axios.put(`/api/pboard/user/like`, JSON.stringify(data),{
        headers:{
          'Content-Type':"application/json"
        }
      })
      :
      await axios.patch(`/api/pboard/user/dislike`, JSON.stringify(data),{
        headers:{
          'Content-Type':"application/json"
        }
      });
    ...
    <>
        ...
        <Col md="auto" style={{ marginTop: 60 }}>
          ...
          <Card border='success' bg='light' style={{ width: '25rem', marginRight:40,height: 700, padding: 10 }}>
            ...
            <ButtonGroup>
              <Button className='btn-10' onClick={sessionStorage.getItem('uid') && onLikeClick} variant="primary">
                <img src={
